...
class ClansPagingSource(
    private val service: ClansApi
) : PagingSource<Int, Clan>() {
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val data = service.getAllClans(page = currentPage.toString())
            LoadResult.Page(
                data = data.clans,
                prevKey = if (currentPage == START_PAGE) null else currentPage - 1,
                nextKey = if (data.clans.isEmpty()) null else currentPage.plus(1)
            )
        } catch (e: Exception) {
            println(e.message)
    ...
    override fun getRefreshKey(state: PagingState<Int, Clan>): Int? {
