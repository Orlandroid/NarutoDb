...
class AkatsukiPagingSource(
    private val service: AkatsukiApi
) : PagingSource<Int, Akatsuki>() {
    ...
    ): LoadResult<Int, Akatsuki> {
        return try {
            val currentPage = params.key ?: START_PAGE
            val data = service.getAllAkatsuki(page = currentPage.toString())
            LoadResult.Page(
                data = data.akatsuki,
                prevKey = if (currentPage == START_PAGE) null else currentPage - 1,
                nextKey = if (data.akatsuki.isEmpty()) null else currentPage.plus(1)
            )
        } catch (e: Exception) {
            println(e.message)
    ...
    override fun getRefreshKey(state: PagingState<Int, Akatsuki>): Int? {
