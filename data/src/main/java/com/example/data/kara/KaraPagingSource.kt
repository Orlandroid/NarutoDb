...
class KaraPagingSource(
    private val service: KaraApi
) : PagingSource<Int, Kara>() {
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val data = service.getAllAkara(page = currentPage.toString())
            LoadResult.Page(
                data = data.kara,
                prevKey = if (currentPage == START_PAGE) null else currentPage - 1,
                nextKey = if (data.kara.isEmpty()) null else currentPage.plus(1)
            )
        } catch (e: Exception) {
            println(e.message)
    ...
    override fun getRefreshKey(state: PagingState<Int, Kara>): Int? {
